...
import { Shield, Award, CheckCircle, Star, Leaf, Mail, Phone, MapPin, Instagram, Twitter, Facebook } from 'lucide-react';
...
export function Footer() {
  ...
  const cms = useCms();
  const accountLinks = FOOTER_LINKS.account.filter(link => !(link.requiresGuest && isAuthenticated));
  const certBadges = cms['cms.footerCertificates'].filter(c => c.active).slice(0, 6);
  return (
    <footer>
      {/* Main footer */}
      <div style={{ background: 'linear-gradient(135deg, #0D2D22 0%, #1A3D30 100%)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 sm:pt-16 pb-8 sm:pb-10">

          {/* Top row: Brand + newsletter */}
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-20 pb-8 sm:pb-12 border-b border-white/10">
            {/* Brand */}
            <div className="lg:max-w-xs shrink-0">
              <Link href="/" className="flex items-center gap-2.5 mb-4 group">
                ...
                <div>
                  <span className="font-serif font-bold text-xl text-white leading-none block">Numa Fresh</span>
                  <span className="text-white/40 text-[10px] tracking-widest uppercase">Halal Marketplace</span>
                </div>
              </Link>
              <p className="text-white/60 text-sm leading-relaxed mb-5">
                America's trusted halal grocery marketplace. Fresh, certified, and delivered with care to Muslim families nationwide.
              </p>
                  ...
                  {certBadges.map(badge => {
                    ...
                    const inner = (
                      <div className="flex flex-col items-center gap-1 px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                        ...
                        <span className="text-white text-[10px] font-bold leading-none">{badge.label}</span>
                        <span className="text-white/40 text-[9px]">{badge.sub}</span>
                      </div>
                    );
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 flex-1">
              <div>
                <h4 className="text-white font-semibold text-sm mb-4">My Account</h4>
              ...
              <div>
                <h4 className="text-white font-semibold text-sm mb-4">For Businesses</h4>
                <ul className="space-y-2.5">
              ...
              <div id="contact">
                ...
                <ul className="space-y-2.5">
                  <li>
                    <a href="mailto:numasetup@gmail.com" className="footer-contact-item flex items-start gap-2 text-white/55 text-sm hover:text-white transition-colors leading-snug">
                      <Mail className="w-3.5 h-3.5 text-[#3FB196] mt-0.5 shrink-0" />
                      <span className="break-all">numasetup@gmail.com</span>
                    </a>
                  </li>
                  <li>
                    <a href="tel:+15712645687" className="footer-contact-item flex items-center gap-2 text-white/55 text-sm hover:text-white transition-colors">
                      <Phone className="w-3.5 h-3.5 text-[#3FB196] shrink-0" /> +1 (571) 264-5687
                    </a>
                  </li>
                  <li>
                    <div className="flex items-start gap-2 text-white/55 text-sm">
                      <MapPin className="w-3.5 h-3.5 text-[#3FB196] mt-0.5 shrink-0" />
                      <span className="leading-snug">4773 Charter Ct,<br />Woodbridge VA, USA</span>
                    </div>
                  </li>
                </ul>
              </div>
